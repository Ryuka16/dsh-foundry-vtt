...
function deepPick(src: unknown, tree: Tree): unknown {
  if (tree === true) return src
 ...
 */
export function summarizeDoc(doc: unknown): unknown {
  if (doc == null || typeof doc !== 'object') return doc
  const d = doc as Record<string, unknown>
  const type = String(d.type ?? '')
  if (type === 'npc' || type === 'character' || type === 'vehicle') return deepPick(d, ACTOR_TREE)
  // relay 的 scene 文档顶层常缺 type 字段，用 grid 特征判定
  if (type === 'scene' || (d.grid && typeof d.grid === 'object')) return deepPick(d, SCENE_TREE)
  const sys = d.system
  if (sys && typeof sys === 'object') {
    const s = sys as Record<string, unknown>
    if ('activities' in s || 'damage' in s) return deepPick(d, ITEM_TREE)
  }
