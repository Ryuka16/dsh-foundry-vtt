...
> 来源: https://xcnplziulnma.feishu.cn/wiki/GBLNw30Biiq6iKk8DF8cx4oMnAb

本项目改进自 @sora 的原始程序。
本文转载自   老师所撰写的Github说明，以下为原文地址：
🔗 书签: 
📎 文件: 
📎 文件: 
📎 文件: 
📎 文件: 
注意，FVTT 14.357以上版本必须安装对应的fix版本补丁！否则模组管理器会无法加载分类内容！
## 目前只兼容FVTT v14、v13与v12！其他版本安装会正常启动但无法识别数据；注意选择正确的版本，版本错误会导致启动后无法读取数据！可卸载补丁(不是卸载FVTT)还原。
## 如何安装
1. 下载上述压缩包至服务器任意区域；
1. 解压缩包，进入文件夹；
1. 双击点击install.bat(Linux先赋权在运行sh脚本)，输入FVTT的程序文件夹路径(注意，是程序的目录文件夹，不是含Data的数据文件夹！也就是那个含有exe文件和许多dll/js文件的文件夹)
> 💡 
右键桌面的FVTT快捷方式，选择属性，复制其中的起始位置就是程序文件夹路径
![图片](images/ZAk9biAyWoGoFnxmpLjcee1Tnff.png)
1. 回车继续运行，程序会自动备份源文件并打上补丁。
## 如何卸载
1. 同安装
1. 同安装
1. 双击点击uninstall.bat(Linux赋权再运行)，选择卸载与还原备份即可。
不想研究部署和原理不用看下面。
——————
# FVTT 下载加速代理（自建方法，用户无需查看）
...
1. 服务器位置 - 建议部署在能快速访问 GitHub 的地区（如香港、日本、美国）
1. FVTT API 是 POST - /api/fvtt/packages 是 POST 请求，浏览器直接访问会 404