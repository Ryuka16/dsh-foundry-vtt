...
export function registerExtraTools(h: ExtraHelpers, reg: Reg): void {
  ...
  reg(h.makeTool(
    ...
    async (args: Args) => {
      ...
      const buildScript = (ws: string[]): string =>
        'const words = ' + JSON.stringify(ws) + ';\n' +
        'const scan = ' + scan + ';\n' +
        'const seen = {};\n' +
        'const out = [];\n' +
      ...
      const runSearch = async (ws: string[]): Promise<Array<Record<string, unknown>>> => {
        ...
        const inner = env.result && typeof env.result === 'object' && !Array.isArray(env.result)
          ...
          : env
        return Array.isArray(inner.entries) ? (inner.entries as Array<Record<string, unknown>>) : []
      }
  ...
  reg(h.makeTool('foundry_create_batch',
    '批量创建实体。★内建分批与间隔，避开三条实测硬约束：①逐条 HTTP 创建会拖垮 relay（408）②世界内一次创建几百条会静默返回 0（一条不写还不报错）③超时其实已落库、别急着重跑。默认每批 30 条、批间隔 90ms。优先走世界内循环（一次 HTTP 调用搞定，实测 relay 的 /create 不接受数组），世界未开 execute-js 时自动退回逐条 HTTP。',
    {
      entityType: {
        type: 'string',
    ...
    async (args) => {
      ...
      if (!docs.length) return { error: 'documents 为空，没有东西可建' }
      ...
      try {
        ...
        const raw = ((env as Record<string, unknown>).result && typeof (env as Record<string, unknown>).result === 'object')
          ? (env as Record<string, unknown>).result as Record<string, unknown>
      ...
      const created2: Array<Record<string, unknown>> = []
      const failed2: Array<Record<string, unknown>> = []
      const batches2: Array<Record<string, unknown>> = []
      for (let i = 0; i < docs.length; i += bs) {
        const slice = docs.slice(i, i + bs)
        for (const d of slice) {
          const bodyBase: Record<string, unknown> = { entityType: et, data: d }
          if (fol) bodyBase.folder = fol
          try {
            const r = h.asObject(await h.callRelay('POST', '/create', { body: bodyBase })) as Record<string, unknown>
            const ent = (r.entity && typeof r.entity === 'object') ? r.entity as Record<string, unknown> : r
            created2.push({ id: ent._id ?? r.uuid ?? '', name: ent.name ?? (d as Record<string, unknown>).name ?? '' })
          } catch (e) {
            failed2.push({ name: (d as Record<string, unknown>).name ?? '', error: String(e) })
          }
        }
  ...
  reg(h.makeTool('foundry_list_packs',
    '列出世界里的全部 compendium 包（id + 标题 + 类型 + 条目数）。★实测：世界里 325 个包里【有多个包标题是空字符串】（第三方模组没写 title），只按包名关键词筛会漏 → 会误报「你世界没装这个包」。本工具同时按 id 与标题匹配，并回报空标题包的数量。需要世界开启 execute-js。',
    {
    ...
    async (args) => {
      ...
      const env = h.asObject(await h.callRelay('POST', '/execute-js', { body: { script } })) as Record<string, unknown>
      const raw = (env.result && typeof env.result === 'object') ? env.result as Record<string, unknown> : env
      const all = Array.isArray(raw.packs) ? raw.packs as Array<Record<string, unknown>> : []
      let list = all
      if (kw) list = list.filter(p => String(p.id ?? '').toLowerCase().includes(kw) || String(p.title ?? '').toLowerCase().includes(kw))
      if (ty) list = list.filter(p => String(p.type ?? '') === ty)
  ...
  reg(h.makeTool('foundry_cleanup_orphan_effects',
    '扫描「断链的活动级效果引用」：物品上某个活动的 effects[]._id 指向的效果已经不存在。★根因：删效果只删效果本身，活动里存的是纯字符串 id，不会自动清理 → 活动被使用时取不到效果，静默不施加（不报错，只是打上去没反应）。实测另一会话删 51 个 AE 后有 49 条物品的活动还指着已删 id。默认只列不删。需要世界开启 execute-js。',
    {
    ...
    async (args) => {
      ...
      const env = h.asObject(await h.callRelay('POST', '/execute-js', { body: { script } })) as Record<string, unknown>
      const raw = (env.result && typeof env.result === 'object') ? env.result as Record<string, unknown> : env
      if (raw.error) return { error: true, message: raw.error }
      return {
