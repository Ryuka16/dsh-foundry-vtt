...
> ⚠️ **关于上面模板里的 `"otherActivityId": ""`**（这个值是原样导出的，别把它当成该照抄的习惯）：midi-qol 里 `attack` 的该字段**默认就是空串 = 自动探测（auto）**——只要本 item 上还有其它类型合格（含 `utility`）且 `otherActivityCompatible` 为真的活动，就会被自动绑到这次攻击上，**点一次攻击连带别人的豁免与伤害**（实测翻车点，《挽歌》那次）。
> **需要各活动独立时，显式写 `"none"`（不是 `""`）。** 用本插件 `foundry_create_item_minimal` 建出来的活动已经是 `"none"`，无需手改；只有手搓 JSON 时才要自己写。详见知识库 `FVTT-monster-spec-v2_1.md` 的勘误块。
