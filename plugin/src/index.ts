...
interface Cfg {
  ...
  clientId?: string
  knowledgeDir?: string
  /** relay.exe 绝对路径。配置了则 DSH 启动时自动确保 relay 在跑（没跑就静默拉起，直接起 exe 不经脚本，避免 PowerShell 5.1 读 UTF-8 脚本乱码）。 */
  relayExePath?: string
  /** relay 数据目录（DATA_DIR），默认 = exe 同目录下的 data。 */
  relayDataDir?: string
  /** 首次启动自动建管理员账号用（已有数据库时无副作用）。 */
  relayAdminEmail?: string
  relayAdminPassword?: string
  /** relay 健康检查地址，默认取 relayUrl。 */
...
async function callRelay(
  ...
  try {
    ...
    throw new HttpError(
      '无法连接 relay：' + (e instanceof Error ? e.message : String(e)) +
        '。请确认 relay 已启动（浏览器打开 ' + cfg.relayUrl.replace(/\/$/, '') + '/api/health 应返回 ok）且 FVTT 世界页面开着。',
      0,
      e,
    )
  }
 ...
 * （朋友那台实测为此绕了近一小时）。
 */
async function diagnoseRelayStall(cfg: ResolvedCfg): Promise<string> {
  ...
  try {
    ...
    if (r.ok) {
      const j = (await r.json()) as { clients?: Array<Record<string, unknown>> }
      const c = (j.clients ?? [])[0]
      if (c) {
        const seen = Number(c.lastSeen ?? 0)
        const age = seen > 0 ? Math.round((Date.now() - seen) / 1000) : -1
        out.push(
          '【自诊断】relay 侧仍登记着客户端 world=' + String(c.worldId ?? '?') +
            (age >= 0 ? '（最后心跳 ' + age + ' 秒前）' : '') +
 ...
 */
async function mintPairingCode(worldId: string, worldTitle: string, systemId: string): Promise<Record<string, unknown>> {
  const cfg = await getCfg()
  const base = cfg.relayUrl.replace(/\/$/, '')
  if (!cfg.relayAdminEmail || !cfg.relayAdminPassword) {
    throw new HttpError(
      '未配置 relay 管理员账号，无法自动出码。请在 ' + CONFIG_FILE +
  ...
  const call = async (path: string, init: RequestInit = {}): Promise<Record<string, unknown>> => {
    ...
    try { body = JSON.parse(text) } catch { /* 非 JSON 响应保留原文 */ }
    if (!res.ok) {
  ...
  if (!code) throw new HttpError('未拿到配对请求 code：' + JSON.stringify(pr).slice(0, 200))
  ...
  return {
    ...
    instruction:
      '把 ' + pairingCode + ' 给用户，让他在 FVTT 模块设置 → REST API Connection → Manage Connection 里点「Enter Code」输入（5 分钟内有效）。' +
...
async function ensureRelay(): Promise<void> {
  ...
  if (!exe) return // 未配置 relayExePath：不自动拉起（发布版默认，用户按需开启）
  ...
  if (cfg.relayAdminEmail) env.ADMIN_EMAIL = cfg.relayAdminEmail
  if (cfg.relayAdminPassword) env.ADMIN_PASSWORD = cfg.relayAdminPassword
  try {
...
export function apply(ctx: any): void {
  ...
  REG(makeTool(
    'foundry_mint_pairing_code',
    '一键生成 6 位配对码：用 config.json 里的 relay 管理员账号自动走完 relay 配对流程，返回配对码 + 给用户看的操作指引。**用户说「给我配对码」「新世界要重新配对」「模块装好了要配对」时直接调这个工具**；不要去磁盘上翻教程文件，更不要自己写 PowerShell 脚本去实现（会被 Windows 执行策略拦，实测三次全失败）。前提：config.json 里已配 relayAdminEmail / relayAdminPassword。',
    {
    ...
    async (args: Record<string, unknown>) =>
      mintPairingCode(
        String(args.worldId ?? 'unknown'),
        String(args.worldTitle ?? ''),
        String(args.systemId ?? 'dnd5e'),
      ),
  ))
  ...
  REG(makeTool(
    'foundry_inspect',
    '一次读回一个实体的**指定字段值**（只读，不比对）。用途：写完之后想确认某几个字段到底变成了什么，又不想拉整份文档。paths 用点号路径 + [n] 下标（语法同 foundry_diff），例：["system.damage.base.denomination","system.activities.dnd5eactivity100.save.dc.formula","flags.autoanimations.primary.video.customPath"]。labels=true 会额外取 dnd5e **运行时算出来的** labels —— 这是「写进去的」与「算出来的」的唯一对照口径（写进去的 1d8 也可能被算成 2d10，只有 labels 看得见），需要世界开着 execute-js。没找到的路径会进 missing，不会静默消失。',
    {
    ...
    async (args) => {
      ...
      if (!paths.length && !wantLabels) return { error: '至少要给 paths（要读哪些字段）或 labels:true' }
      const raw = await callRelay('GET', '/get', { query: { ...targetingQuery(args), uuid } })
      const rec = raw as Record<string, unknown> | undefined
      const doc =
        (rec && typeof rec.entity === 'object' && rec.entity) ||
      ...
      if (wantLabels) {
        try {
          ...
          const inner = ((env as Record<string, unknown>)?.result ?? env) as Record<string, unknown>
          out.labels = inner?.labels ?? null
          if (inner?.found === false) out.labelsNote = 'fromUuid 没解析到这个 uuid（可能是不在世界里的 compendium 实体）'
        } catch (e) {
          out.labelsNote =
            'labels 读取失败：' +
