 ...
 */
function scanIndexPaths(v, prefix = '', out = []) {
        ...
        try {
            ...
            if (args.uuid && e instanceof HttpError) {
                ...
                if (/does not exist|failed to update entity/i.test(rawErr)) {
                    const ff = { mutation: 'update', uuid: args.uuid, verified: true, changed: args.data, note: '模块回读确认失败但写入已执行（假阴性）——判定已生效。' };
                    if (idxWarn)
                        ff.indexPathWarning = idxWarn;
                    return ff;
                }
                try {
