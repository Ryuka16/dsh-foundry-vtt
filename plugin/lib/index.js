...
export function apply(ctx) {
    ...
    REG(makeTool('foundry_list_worlds', '列出连接 relay 的所有 Foundry 世界/客户端（含在线状态、systemId、systemVersion、foundryVersion）。第一步确认哪个世界在线、dnd5e 版本。', {}, [], async () => {
    ...
    REG(makeTool('foundry_manage_folder', '创建或删除 Foundry 文件夹。create：name + folderType(文档类)。创建后把返回的 uuid 作 folder 参数传给 create_entity。delete：folderId（deleteAll=true 连带删除内部实体，不可逆）。', {
        ...
        if (args.action === 'create') {
            ...
            if (args.parentFolderId)
                q.parentFolderId = args.parentFolderId;
