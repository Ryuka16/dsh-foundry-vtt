...
function scanForbidden(script) {
    const hits = [];
    const lines = script.split('\n');
    for (let i = 0; i < lines.length; i++) {
        for (const [label, re] of FORBIDDEN_PATTERNS) {
            if (re.test(lines[i]))
                hits.push({ pattern: label, line: i + 1, text: lines[i].trim().slice(0, 160) });
        }
    }
    return hits;
}
function bodyOf(args, keys) {
    const b = {};
    for (const k of keys)
        if (args[k] !== undefined)
            b[k] = args[k];
...
function simple(h, name, desc, method, path, props, required, bodyKeys = [], queryKeys = [], pathKeys = []) {
    return h.makeTool(name, desc, props, required, async (args) => {
        let p = path;
        ...
        if (bodyKeys.length > 0)
            opts.body = bodyOf(args, bodyKeys);
...
export function registerExtraTools(h, reg) {
    // ═══ dnd5e 系统操作（21 工具）═══════════════════════════════════
    reg(simple(h, 'foundry_dnd5e_ability_check', '为 actor 掷属性检定（POST /dnd5e/ability-check）。ability 用缩写 str/dex/con/int/wis/cha；advantage/disadvantage 二选一；bonus 为额外加值公式。', 'POST', '/dnd5e/ability-check', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, ability: { type: 'string', enum: ['str', 'dex', 'con', 'int', 'wis', 'cha'], description: '属性缩写' }, ...P_adv }, ['actorUuid', 'ability'], ['actorUuid', 'ability', 'advantage', 'disadvantage', 'bonus', 'createChatMessage']));
    reg(simple(h, 'foundry_dnd5e_ability_save', '为 actor 掷属性豁免（POST /dnd5e/ability-save）。ability 用缩写 str/dex/con/int/wis/cha。', 'POST', '/dnd5e/ability-save', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, ability: { type: 'string', enum: ['str', 'dex', 'con', 'int', 'wis', 'cha'], description: '属性缩写' }, ...P_adv }, ['actorUuid', 'ability'], ['actorUuid', 'ability', 'advantage', 'disadvantage', 'bonus', 'createChatMessage']));
    reg(simple(h, 'foundry_dnd5e_skill_check', '为 actor 掷技能检定（POST /dnd5e/skill-check）。skill 用 dnd5e 技能缩写（如 acr/ath/ste/prc/ins/per）。', 'POST', '/dnd5e/skill-check', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, skill: { type: 'string', description: '技能缩写（acr/ath/ste/prc/ins/per 等）' }, ...P_adv }, ['actorUuid', 'skill'], ['actorUuid', 'skill', 'advantage', 'disadvantage', 'bonus', 'createChatMessage']));
    reg(simple(h, 'foundry_dnd5e_death_save', '为 actor 掷死亡豁免（POST /dnd5e/death-save），自动累计成功/失败次数。', 'POST', '/dnd5e/death-save', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, advantage: { type: 'boolean', description: '优势' }, createChatMessage: { type: 'boolean', description: '是否发聊天窗（默认 true）' } }, ['actorUuid'], ['actorUuid', 'advantage', 'createChatMessage']));
    reg(simple(h, 'foundry_dnd5e_concentration_save', '为 actor 掷专注豁免（POST /dnd5e/concentration-save），伤害数值用于计算 DC（DC=max(10, 伤害/2 向下取整)）。失败自动打断专注。', 'POST', '/dnd5e/concentration-save', { ...P_actorActorName, damage: { type: 'number', description: '受到的伤害值（算 DC 用）' }, advantage: { type: 'boolean', description: '优势' }, disadvantage: { type: 'boolean', description: '劣势' }, bonus: { type: 'string', description: '额外加值公式' }, createChatMessage: { type: 'boolean', description: '是否发聊天窗（默认 true）' } }, ['damage'], ['actorUuid', 'actorName', 'damage', 'advantage', 'disadvantage', 'bonus', 'createChatMessage']));
    reg(simple(h, 'foundry_dnd5e_concentration', '查 actor 当前是否在专注某个法术（GET /dnd5e/concentration），返回专注的法术信息。', 'GET', '/dnd5e/concentration', { ...P_actorActorName }, [], [], ['actorUuid', 'actorName']));
    reg(simple(h, 'foundry_dnd5e_break_concentration', '主动打断 actor 的专注（POST /dnd5e/break-concentration）。', 'POST', '/dnd5e/break-concentration', { ...P_actorActorName }, [], ['actorUuid', 'actorName']));
    reg(simple(h, 'foundry_dnd5e_get_actor_details', '读取 dnd5e actor 的详细信息（GET /dnd5e/get-actor-details）。details 为详情类型数组，如 ["resources","items","spells","features","prof"]——按需组合。', 'GET', '/dnd5e/get-actor-details', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, details: { type: 'array', items: { type: 'string' }, description: '详情类型数组（resources/items/spells/features/prof 等）' } }, ['actorUuid', 'details'], [], ['actorUuid', 'details']));
    ...
    reg(simple(h, 'foundry_dnd5e_equip_item', '装备/卸下 actor 的物品（POST /dnd5e/equip-item）。equipped=true 装备、false 卸下；自动联动 AC/伤害/熟练。', 'POST', '/dnd5e/equip-item', { ...P_actorActorName, ...P_item, equipped: { type: 'boolean', description: 'true 装备 / false 卸下' } }, ['equipped'], ['actorUuid', 'actorName', 'itemUuid', 'itemName', 'equipped']));
    reg(simple(h, 'foundry_dnd5e_attune_item', '调谐/取消调谐物品（POST /dnd5e/attune-item）。attuned=true 调谐、false 取消。', 'POST', '/dnd5e/attune-item', { ...P_actorActorName, ...P_item, attuned: { type: 'boolean', description: 'true 调谐 / false 取消调谐' } }, ['attuned'], ['actorUuid', 'actorName', 'itemUuid', 'itemName', 'attuned']));
    reg(simple(h, 'foundry_dnd5e_prepare_spell', '准备/取消准备法术（POST /dnd5e/prepare-spell）。spellName 为法术名，prepared=true 准备。', 'POST', '/dnd5e/prepare-spell', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, spellName: { type: 'string', description: '法术名' }, prepared: { type: 'boolean', description: 'true 准备 / false 取消' } }, ['actorUuid', 'spellName', 'prepared'], ['actorUuid', 'spellName', 'prepared']));
    reg(simple(h, 'foundry_dnd5e_long_rest', '为 actor 执行长休（POST /dnd5e/long-rest）：回满 HP/恢复法术位/恢复特性。newDay 默认 true（换天）。', 'POST', '/dnd5e/long-rest', { ...P_actorActorName, selected: { type: 'boolean', description: '作用于当前选中的 token' }, newDay: { type: 'boolean', description: '是否算新的一天（默认 true）' } }, [], ['actorUuid', 'actorName', 'selected', 'newDay']));
    reg(simple(h, 'foundry_dnd5e_short_rest', '为 actor 执行短休（POST /dnd5e/short-rest）：可自动花生命骰回血（autoHD + autoHDThreshold 0-1）。', 'POST', '/dnd5e/short-rest', { ...P_actorActorName, selected: { type: 'boolean', description: '作用于当前选中的 token' }, autoHD: { type: 'boolean', description: '自动花生命骰' }, autoHDThreshold: { type: 'number', description: '低于该 HP 比例（0-1）自动花骰' } }, [], ['actorUuid', 'actorName', 'selected', 'autoHD', 'autoHDThreshold']));
    reg(simple(h, 'foundry_dnd5e_modify_currency', '增减 actor 的钱币（POST /dnd5e/modify-currency，delta 式）。currency 用 pp/gp/ep/sp/cp；amount 正数加、负数减。', 'POST', '/dnd5e/modify-currency', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, currency: { type: 'string', enum: ['pp', 'gp', 'ep', 'sp', 'cp'], description: '钱币面额' }, amount: { type: 'number', description: '增减数量（正加负减）' } }, ['actorUuid', 'currency', 'amount'], ['actorUuid', 'currency', 'amount']));
    reg(simple(h, 'foundry_dnd5e_transfer_currency', '两个 actor 之间转移钱币（POST /dnd5e/transfer-currency）。currency 为对象如 {"gp":10,"sp":5}，从 source 转到 target。', 'POST', '/dnd5e/transfer-currency', {
    ...
    }, ['currency'], ['sourceActorUuid', 'sourceActorName', 'targetActorUuid', 'targetActorName', 'currency']));
    reg(simple(h, 'foundry_dnd5e_modify_experience', '增减 actor 的经验值（POST /dnd5e/modify-experience）。amount 可正可负。', 'POST', '/dnd5e/modify-experience', { ...P_actorActorName, selected: { type: 'boolean', description: '作用于当前选中的 token' }, amount: { type: 'number', description: '增减经验值（可负数）' } }, ['amount'], ['actorUuid', 'actorName', 'selected', 'amount']));
    reg(simple(h, 'foundry_dnd5e_modify_item_charges', '增减 actor 某物品的充能次数（POST /dnd5e/modify-item-charges）。amount 正加负减。', 'POST', '/dnd5e/modify-item-charges', { actorUuid: { type: 'string', description: 'actor 的 uuid' }, ...P_item, amount: { type: 'number', description: '增减充能数（正加负减）' } }, ['actorUuid', 'amount'], ['actorUuid', 'itemUuid', 'itemName', 'amount']));
    // ═══ 遭遇与回合（9 工具）════════════════════════════════════════
    reg(simple(h, 'foundry_encounter_list', '列出当前进行中的战斗遭遇（GET /encounters），含回合数/当前回合/参战者与先攻顺序。', 'GET', '/encounters', {}, [], [], []));
    reg(simple(h, 'foundry_encounter_start', '开始一场新的战斗遭遇（POST /start-encounter）。tokens 为 token uuid 数组；startWithSelected=true 用当前选中的 token；rollAll/rollNPC 自动掷先攻。', 'POST', '/start-encounter', {
    ...
    reg(simple(h, 'foundry_encounter_end', '结束当前战斗遭遇（POST /end-encounter）。encounter 缺省=当前遭遇。', 'POST', '/end-encounter', { encounter: { type: 'string', description: '遭遇 id（缺省=当前遭遇）' } }, [], ['encounter']));
    reg(simple(h, 'foundry_encounter_add', '把 token 加入当前遭遇（POST /add-to-encounter）。uuids 为 token uuid 数组；rollInitiative 加入即掷先攻。', 'POST', '/add-to-encounter', { uuids: { type: 'array', items: { type: 'string' }, description: 'token uuid 数组' }, selected: { type: 'boolean', description: '加入当前选中的 token' }, encounter: { type: 'string', description: '遭遇 id（缺省=当前）' }, rollInitiative: { type: 'boolean', description: '加入后掷先攻' } }, [], ['uuids', 'selected', 'encounter', 'rollInitiative']));
    reg(simple(h, 'foundry_encounter_remove', '把 token 移出当前遭遇（POST /remove-from-encounter）。', 'POST', '/remove-from-encounter', { uuids: { type: 'array', items: { type: 'string' }, description: 'token uuid 数组' }, selected: { type: 'boolean', description: '移出当前选中的 token' }, encounter: { type: 'string', description: '遭遇 id（缺省=当前）' } }, [], ['uuids', 'selected', 'encounter']));
    ...
    // ═══ 场景与画布（11 工具）═══════════════════════════════════════
    reg(simple(h, 'foundry_scene_create', '新建一个场景（POST /scene）。data 为场景文档对象 {name,width,height,grid:{size},background:{src} 等}。', 'POST', '/scene', { data: { type: 'object', description: '场景数据对象（name/width/height/grid/background 等）' } }, ['data'], ['data']));
    reg(simple(h, 'foundry_scene_update', '更新场景（PUT /scene）。sceneId/name 二选一定位；data 为要改的字段。', 'PUT', '/scene', { sceneId: { type: 'string', description: '场景 id' }, name: { type: 'string', description: '场景名' }, active: { type: 'boolean', description: '直接指向激活场景' }, data: { type: 'object', description: '要更新的字段对象' } }, ['data'], ['sceneId', 'name', 'active', 'data']));
    reg(simple(h, 'foundry_scene_delete', '⚠️ 删除场景（DELETE /scene，不可逆）。sceneId/name 二选一。删除前必须与用户确认。', 'DELETE', '/scene', { sceneId: { type: 'string', description: '场景 id' }, name: { type: 'string', description: '场景名' } }, [], [], ['sceneId', 'name']));
    reg(simple(h, 'foundry_scene_switch', '切换当前激活场景（POST /switch-scene）。sceneId/name 二选一。这是把玩家/GM 视图切到某张地图的操作。', 'POST', '/switch-scene', { sceneId: { type: 'string', description: '要激活的场景 id' }, name: { type: 'string', description: '要激活的场景名' } }, [], ['sceneId', 'name']));
    ...
    reg(simple(h, 'foundry_canvas_get', '读画布上的内嵌文档（GET /canvas/{documentType}）：tokens 读 token 列表、walls 读墙、lights 读光源、drawings/notes/tiles/sounds 同理。sceneId 缺省=激活场景。', 'GET', '/canvas/{documentType}', { ...P_docType, sceneId: { type: 'string', description: '场景 id（缺省=激活场景）' }, documentId: { type: 'string', description: '指定文档 id（缺省=全部）' } }, ['documentType'], [], ['sceneId', 'documentId'], ['documentType']));
    reg(simple(h, 'foundry_canvas_create', '在画布上创建内嵌文档（POST /canvas/{documentType}）。data 为该文档类型的数据对象（token 用 {x,y,actorId,...}、墙用 {c:[x1,y1,x2,y2],...}）。sceneId 缺省=激活场景。', 'POST', '/canvas/{documentType}', { ...P_docType, data: { type: 'object', description: '文档数据对象' }, sceneId: { type: 'string', description: '场景 id（缺省=激活场景）' } }, ['documentType', 'data'], ['data', 'sceneId'], [], ['documentType']));
    reg(simple(h, 'foundry_canvas_update', '更新画布内嵌文档（PUT /canvas/{documentType}）。documentId 为文档 id，data 为要改的字段。', 'PUT', '/canvas/{documentType}', { ...P_docType, documentId: { type: 'string', description: '文档 id' }, data: { type: 'object', description: '要更新的字段' }, sceneId: { type: 'string', description: '场景 id（缺省=激活场景）' } }, ['documentType', 'documentId', 'data'], ['documentId', 'data', 'sceneId'], ['documentId', 'sceneId'], ['documentType']));
    reg(simple(h, 'foundry_canvas_delete', '⚠️ 删除画布内嵌文档（DELETE /canvas/{documentType}，不可逆）。删除前与用户确认。', 'DELETE', '/canvas/{documentType}', { ...P_docType, documentId: { type: 'string', description: '文档 id' }, sceneId: { type: 'string', description: '场景 id（缺省=激活场景）' } }, ['documentType', 'documentId'], [], ['documentId', 'sceneId'], ['documentType']));
    reg(simple(h, 'foundry_measure_distance', '测量两点/两个 token 之间的距离（GET /measure-distance）。坐标对（originX/Y 与 targetX/Y）或 token 对（uuid/name）二选一。', 'GET', '/measure-distance', {
    ...
    reg(simple(h, 'foundry_select', '选中画布上的 token（POST /select）。uuids 数组、name 名、all 全选、data 按数据匹配（如 {"attributes.hp.value":20}）。', 'POST', '/select', { uuids: { type: 'array', items: { type: 'string' }, description: '要选中的 uuid 数组' }, name: { type: 'string', description: '按名字选中' }, all: { type: 'boolean', description: '全选' }, overwrite: { type: 'boolean', description: '覆盖现有选择' }, data: { type: 'object', description: '按数据匹配选择' } }, [], ['uuids', 'name', 'all', 'overwrite', 'data']));
    reg(simple(h, 'foundry_get_selected', '读取画布上当前选中的 token（GET /selected）。', 'GET', '/selected', {}, [], [], []));
    // ═══ 聊天（4 工具）══════════════════════════════════════════════
    reg(simple(h, 'foundry_chat_send', '发送聊天消息（POST /chat）。content 支持 HTML；speaker 传 actor id 会以该角色名义说话；whisper 传用户 id 数组则私聊；chatType：0=OOC 1=IC 2=Emote 3=Whisper 4=Roll。', 'POST', '/chat', {
    ...
    }, ['content'], ['content', 'flavor', 'speaker', 'whisper', 'alias', 'chatType']));
    reg(simple(h, 'foundry_chat_get', '读聊天记录（GET /chat）。limit 条数（默认 10）、offset 翻页、chatType 过滤类型、speaker 过滤说话者。', 'GET', '/chat', { limit: { type: 'number', description: '条数（默认 10）' }, offset: { type: 'number', description: '翻页偏移' }, chatType: { type: 'number', description: '0=OOC 1=IC 2=Emote 3=Whisper 4=Roll' }, speaker: { type: 'string', description: '按说话者过滤' } }, [], [], ['limit', 'offset', 'chatType', 'speaker']));
    reg(simple(h, 'foundry_chat_clear', '⚠️ 清空全部聊天消息（DELETE /chat，不可逆）。执行前与用户确认。', 'DELETE', '/chat', {}, [], [], []));
    reg(simple(h, 'foundry_chat_delete', '删除单条聊天消息（DELETE /chat/{messageId}）。messageId 从 foundry_chat_get 结果里拿。', 'DELETE', '/chat/{messageId}', { messageId: { type: 'string', description: '要删的消息 id' } }, ['messageId'], [], [], ['messageId']));
    // ═══ 用户管理（5 工具）═════════════════════════════════════════
    reg(simple(h, 'foundry_user_list', '列出世界全部用户（GET /users）：id/名字/角色/在线状态/颜色。', 'GET', '/users', {}, [], [], []));
    reg(simple(h, 'foundry_user_get', '按 id 或名字查单个用户（GET /user）。', 'GET', '/user', { id: { type: 'string', description: '用户 id' }, name: { type: 'string', description: '用户名' } }, [], [], ['id', 'name']));
    reg(simple(h, 'foundry_user_create', '新建用户（POST /user）。role：0=无 1=玩家 2=可信 3=助理 4=GM（默认 1）。password 可选。', 'POST', '/user', { name: { type: 'string', description: '用户名' }, password: { type: 'string', description: '密码（可选）' }, role: { type: 'number', description: '0=无 1=玩家 2=可信 3=助理 4=GM' } }, ['name'], ['name', 'password', 'role']));
    reg(simple(h, 'foundry_user_update', '更新用户（PUT /user）。id/name 二选一定位；data 为要改的字段 {name,role,password,color,avatar,...}。', 'PUT', '/user', { id: { type: 'string', description: '用户 id' }, name: { type: 'string', description: '用户名' }, data: { type: 'object', description: '要更新的字段对象' } }, ['data'], ['id', 'name', 'data'], ['id', 'name']));
    reg(simple(h, 'foundry_user_delete', '⚠️ 删除用户（DELETE /user，不可逆）。删除前与用户确认。', 'DELETE', '/user', { id: { type: 'string', description: '用户 id' }, name: { type: 'string', description: '用户名' } }, [], [], ['id', 'name']));
    // ═══ 宏与 JS / 结构（4 工具）═══════════════════════════════════
    ...
    reg(h.makeTool('foundry_execute_js', '⚠️ 在世界内直接执行 JavaScript（POST /execute-js）。这是最高权限的底层操作：可读写世界任意数据、可调用任何 Foundry API，返回任意 JSON。\n' +
        ...
        '**报错排查**：若返回 `Error executing script: <某处的> is not a function` 之类 —— 多半是假设了字段类型（实测踩过 `(p.types || []).join is not a function`，因为 types 是对象不是数组）。先在脚本里 `return { 探到的值: typeof 某字段, 样例: 某字段 }` 探一次真实形状，再写正式逻辑；不要在类型不明时直接 .join()/.map()。', { script: { type: 'string', description: '要执行的 JavaScript 代码（会先过 24 条 forbidden-patterns 预检，命中则不提交并回报行号）' } }, [], async (args) => {
        const script = typeof args.script === 'string' ? args.script : '';
        if (!script.trim())
            return { error: 'script 为空' };
        const hits = scanForbidden(script);
        if (hits.length) {
            return {
                blocked: true,
                submitted: false,
                note: '⚠️ 脚本**未提交**：它会命中 relay 的 forbidden-patterns 校验，必然返回 400 "Script contains forbidden patterns"。' +
        ...
        try {
            ...
            while ((m = re.exec(msg)) !== null) {
                ...
                if (n >= 1 && n <= lines.length && !suspects.includes(n))
                    suspects.push(n);
            }
            ...
            if (tk && suspects.length === 0) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    if (lines[i].indexOf(word) >= 0) {
                        if (!suspects.includes(i + 1))
                            suspects.push(i + 1);
                        if (i > 0 && !suspects.includes(i))
                            suspects.push(i);
                        break;
                    }
                }
    ...
    reg(simple(h, 'foundry_playlist_play', '播放播放列表或其中某条音轨（POST /playlist/play）。soundId/soundName 指定单曲，缺省播整表。', 'POST', '/playlist/play', { ...P_pl, soundId: { type: 'string', description: '音轨 id' }, soundName: { type: 'string', description: '音轨名' } }, [], ['playlistId', 'playlistName', 'soundId', 'soundName']));
    ...
    reg(simple(h, 'foundry_playlist_volume', '设置播放列表音量（POST /playlist/volume）。volume 0.0-1.0；可指定单曲 soundId。', 'POST', '/playlist/volume', { ...P_pl, soundId: { type: 'string', description: '音轨 id（可选）' }, soundName: { type: 'string', description: '音轨名（可选）' }, volume: { type: 'number', description: '音量 0.0-1.0' } }, ['volume'], ['playlistId', 'playlistName', 'soundId', 'soundName', 'volume']));
    reg(simple(h, 'foundry_play_sound', '播放一次性音效（POST /play-sound）。src 为音频路径（如 "sounds/effect.mp3"）；volume 0-1；loop 循环。', 'POST', '/play-sound', { src: { type: 'string', description: '音频文件路径' }, volume: { type: 'number', description: '音量 0-1（默认 0.5）' }, loop: { type: 'boolean', description: '循环播放' } }, ['src'], ['src', 'volume', 'loop']));
    reg(simple(h, 'foundry_stop_sound', '停止音效（POST /stop-sound）。src 缺省=停止全部。', 'POST', '/stop-sound', { src: { type: 'string', description: '要停的音频路径（缺省=全部）' } }, [], ['src']));
    // ═══ 世界信息 / 其他（3 工具）══════════════════════════════════
    // ⚠️ 默认裁掉已装模块清单：第三方实测反馈「foundry_world_info 每次吐回全量模块（实测 204 个），
    // 这一轮我就为它烧掉一大块上下文」。常用的只有世界名 / 系统版本 / Foundry 版本。
    reg(h.makeTool('foundry_world_info', '读世界综合信息（GET /world-info）：世界名/系统版本/Foundry 版本/玩家列表等。⚠️ 已装模块清单**默认省略**（实测 204 个，体积很大）——确实需要完整清单时传 includeModules:true。', { includeModules: { type: 'boolean', description: '是否返回完整已装模块清单（默认 false，只回 moduleCount 与提示）' } }, [], async (args) => {
    ...
    reg(simple(h, 'foundry_get_folder', '按名字查文件夹（GET /get-folder），返回文件夹 uuid（建实体时 folder 参数用）。', 'GET', '/get-folder', { name: { type: 'string', description: '文件夹名' } }, ['name'], [], ['name']));
    reg(simple(h, 'foundry_player_list', '列出玩家/用户简表（GET /players）。', 'GET', '/players', {}, [], [], []));
    ...
    reg(h.makeTool('foundry_search_animation', '动画/音效检索库：在 Sequencer 数据库（jb2a / psfx / blfx 三包）里搜路径，**返回候选给你自己挑**。\n' +
