    ...
    reg(h.makeTool('foundry_patch_item', '改造一个**已有物品**：删活动 / 改活动字段 / 挂物品宏 / 合并任意 flags —— 一次调用完成，**不用写 execute_js**。\n' +
        ...
        if (Array.isArray(args.removeActivities) && args.removeActivities.length) {
            ...
            if (Object.keys(acts).length) {
                ...
                notes.push('删除活动 ' + args.removeActivities.join(', '));
            }
        }
        if (args.activityPatch && typeof args.activityPatch === 'object') {
            const prev = (sysPatch.activities ?? {});
            for (const [id, val] of Object.entries(args.activityPatch)) {
                prev[id] = val;
            }
            sysPatch.activities = prev;
            notes.push('活动字段 merge：' + Object.keys(args.activityPatch).join(', '));
        }
        // ── 物品宏三件套 ──
        const flagPatch = {};
