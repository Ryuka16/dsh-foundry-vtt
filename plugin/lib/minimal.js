 ...
 */
function appendExtraActivities(acts, specs, notes, itemHasUses = false) {
    const made = [];
...
export function registerMinimalTools(h, reg) {
    reg(h.makeTool('foundry_create_item_minimal', '⚠️ **两步走（默认预览，可跳过）**：不带 confirmToken 时**只返回预览、不落库** —— 这是默认动作：把预览里的描述文案 / 数值 / 机制讲给用户听。**用户说「不用看 / 直接建 / 你定」时不要再问**，直接拿预览返回的 confirmToken 立刻调第二次建下去；参数改一个字凭证就失效，会重新回到预览。\n' +
        ...
        '⚠️ **description 必须写全机制，而且整件东西写成一段话**（外观与机制连着写，不要另起条目、不要分小标题 —— 官方物品描述都是一段到底）：段首两三句写外观与来历，紧跟着把机制接上去，**每个效果都要交代全 7 件事** —— ①用什么动作 ②什么时候能用 ③距离或范围 ④对抗方式（DC X 的【属性】豁免）⑤结果（失败受 NdM 点【伤害类型】伤害 / 陷入状态 / 成功减半）⑥持续与重试 ⑦充能与恢复。**活动页玩家未必点开，第一次拿到这东西的人只能靠描述知道它能干什么；只写氛围不写机制＝不合格。** 术语用官方译名（豁免 / 一个动作 / 一个附赠动作 / 充能 / 同调 / 黯蚀——不是「暗蚀」），不要写成代码腔。格式与可套用的官方句式见系统提示的「一之二、物品描述的官方格式」。\n' +
