...
const MIDI_ORIGINAL_OTHER_ID = (kind) => (kind === 'attack' ? '' : 'none');
...
const otherIdPatch = (kind) => MIDI_OTHER_TYPES.includes(kind) ? { otherActivityId: MIDI_ORIGINAL_OTHER_ID(kind) } : {};
/**
 ...
 */
function appendExtraActivities(acts, specs, notes, itemHasUses = false, expectFields = []) {
    const made = [];
    ...
    for (const { id, s, kind } of plan) {
        ...
        const exTargetRaw = (s.target ?? {});
        const exTmpl = (exTargetRaw.template ?? {});
        const exAff = (exTargetRaw.affects ?? {});
        ...
        if (exRangeVal !== '')
            expectFields.push({ path: actPath + 'range.value', want: exRangeVal });
        for (const tk of Object.keys(exTmpl))
            expectFields.push({ path: actPath + 'target.template.' + tk, want: exTmpl[tk] });
        for (const ak of Object.keys(exAff))
            expectFields.push({ path: actPath + 'target.affects.' + ak, want: exAff[ak] });
        const mpIn = (s.midiProperties ?? {});
        if (mpIn && typeof mpIn === 'object' && !Array.isArray(mpIn)) {
            for (const mk of Object.keys(mpIn))
                expectFields.push({ path: actPath + 'midiProperties.' + mk, want: mpIn[mk] });
        }
        ...
        if (kind === 'attack') {
            acts[id] = {
                ...
                type: 'attack',
                attack: {
                    ability: ABILITIES.includes(s.ability) ? String(s.ability) : 'str',
            ...
            const ckAssoc = Array.isArray(ck.associated)
                ? ck.associated.filter((x) => typeof x === 'string' && !!x.trim()).map((x) => x.trim())
                : [];
            acts[id] = {
                ...common,
                type: 'check',
            ...
            const tfArr = (v) => Array.isArray(v)
                ? v.filter((x) => typeof x === 'string' && !!x.trim()).map((x) => x.trim())
                : [];
            const lv = {};
            if (typeof tf.levelMin === 'number' && Number.isFinite(tf.levelMin))
                lv.min = tf.levelMin;
            if (typeof tf.levelMax === 'number' && Number.isFinite(tf.levelMax))
                lv.max = tf.levelMax;
            const profile = {
                _id: ID16(),
            ...
            if (ts.spellLists !== undefined)
                tsObj.spellLists = tfArr(ts.spellLists);
            for (const k of ['tempFormula', 'minimumAC', 'preset']) {
                if (typeof ts[k] === 'string')
                    tsObj[k] = ts[k];
            }
            if (typeof ts.transformTokens === 'boolean')
                tsObj.transformTokens = ts.transformTokens;
            acts[id] = {
                ...common,
                type: 'transform',
                profiles: [profile],
 ...
 */
function confirmTokenFor(args) {
    const clone = {};
    for (const [k, v] of Object.entries(args)) {
        if (k === 'preview' || k === 'confirmToken')
            continue;
        clone[k] = v;
    }
...
function buildRecovery(u, notes) {
    const rec = typeof u.recovery === 'string' && u.recovery.trim()
...
function applyPriceWeight(sys, a, expectFields, notes) {
...
function buildNonWeapon(a, itemType) {
    ...
    const usesObj = a.uses && typeof a.uses === 'object' ? a.uses : undefined;
    if (USES_TYPES.includes(itemType) && (itemType === 'consumable' || usesObj !== undefined)) {
        ...
        if (rr.recPeriods.length)
            expectFields.push({ path: 'system.uses.recovery[0].period', want: rr.recPeriods[0] });
    ...
    const itemHasUses = !!(sys.uses && String(sys.uses.max ?? '') !== '');
    ...
    const checkActKeys = [];
    if (hasHealAct)
        checkActKeys.push('dnd5eactivity000');
    if (wantsUtility)
        checkActKeys.push(hasHealAct ? 'dnd5eactivity100' : 'dnd5eactivity000');
    if (itemType === 'spell')
        checkActKeys.push('dnd5eactivity000');
    for (const k of checkActKeys) {
        if (itemHasUses)
            expectFields.push({ path: 'system.activities.' + k + '.consumption.targets[0].type', want: 'itemUses' });
...
function buildUpdateData(a, attackKey, itemType) {
    ...
    const wUses = a.uses && typeof a.uses === 'object' ? a.uses : undefined;
...
export function registerMinimalTools(h, reg) {
    reg(h.makeTool('foundry_create_item_minimal', '⚠️ **两步走（默认预览，可跳过）**：不带 confirmToken 时**只返回预览、不落库** —— 这是默认动作：把预览里的描述文案 / 数值 / 机制讲给用户听。**用户说「不用看 / 直接建 / 你定」时不要再问**，直接拿预览返回的 confirmToken 立刻调第二次建下去；参数改一个字凭证就失效，会重新回到预览。\n' +
        ...
        if (!skipPreview && (argRec.preview === true || givenToken === '' || givenToken !== expectedToken)) {
            ...
            const pvAsRec = pvData;
            const pvMem = (pvAsRec.system ?? {});
            ...
            const pvEffects = Array.isArray(pvAsRec.effects) ? pvAsRec.effects : [];
            ...
            return {
                ...
                card: {
                    ...
                    img: pvImg,
                    systemType: (pvMem.type ?? null),
                    damage: (pvMem.damage ?? null),
                    rarity: pvMem.rarity ?? null,
                    activities: Object.keys((pvMem.activities ?? {})).map((k) => (itemType === 'weapon' && k === 'dnd5eactivity000')
                    ...
                    description: String((pvMem.description ?? {}).value ?? ''),
                },
                notes: pvNotes,
                warnings: pvWarn,
        ...
        const dataAsRec = data;
        if (Array.isArray(dataAsRec.effects)) {
            for (const e of dataAsRec.effects) {
        ...
        const cprGiven = args.cpr;
        if (cprGiven && typeof cprGiven === 'object' && !Array.isArray(cprGiven) && Object.keys(cprGiven).length > 0) {
            const fl = dataAsRec.flags ?? {};
            const prev = fl['chris-premades'] ?? {};
            fl['chris-premades'] = { ...prev, ...cprGiven };
            dataAsRec.flags = fl;
            ...
            if (actEntries.length >= 2) {
                for (const [k, v] of actEntries) {
                    ...
                    if (av.otherActivityId === '' && String(av.type) === 'attack') {
                        ...
                        notes.push(`提示：活动 ${k}(attack) 的 otherActivityId 是空串 = midi 原生 auto 探测。同 item 上还有 ${actEntries.length - 1} 个活动，点这次攻击时 midi 可能连带结算它们（这是它的默认行为，不是 bug）。若要各活动独立：用 linkedTo 显式串接子活动，或 foundry_patch_item{activityPatch:{"${k}":{"otherActivityId":"none"}}}（注意 none 会同时切断弓弩的弹药链）`);
                    }
                }
            }
    ...
    reg(h.makeTool('foundry_patch_item', '改造一个**已有物品**：删活动 / 改活动字段 / 挂物品宏 / 合并任意 flags —— 一次调用完成，**不用写 execute_js**。\n' +
        ...
        const rmList = Array.isArray(args.removeActivities)
            ? args.removeActivities.filter(x => typeof x === 'string' && x.trim()).map(x => x.trim())
            : [];
        ...
        if (rmList.length || patchObj) {
            const rawCur = (await h.callRelay('GET', '/get', { query: { ...h.targetingQuery(args), uuid: fullUuid } }));
            const cur = unwrapEntity(rawCur);
            const curActs = { ...((cur?.system?.activities) ?? {}) };
            let dirty = false;
            if (rmList.length) {
                const removed = [];
                for (const [k, v] of Object.entries(curActs)) {
                    const vid = String((v?._id) ?? '');
                    if (rmList.includes(k) || (vid && rmList.includes(vid))) {
                        delete curActs[k];
                        removed.push(k);
                    }
                }
                if (removed.length) {
                    dirty = true;
                    notes.push('删除活动 ' + removed.join(', '));
                }
                else
                    notes.push('⚠️ 要删的活动一个都没找到：' + rmList.join(', ') + ' —— 先 foundry_inspect 读 system.activities 的键名（键名 = 活动 _id）');
            }
            if (patchObj) {
                const patched = [];
                for (const [id, val] of Object.entries(patchObj)) {
                    if (!curActs[id]) {
                        notes.push('⚠️ activityPatch 的 ' + id + ' 不在该物品上，已跳过');
                        continue;
                    }
                ...
                if (patched.length) {
                    dirty = true;
                    notes.push('活动字段 merge：' + patched.join(', '));
                }
            }
            if (dirty)
                sysPatch.activities = curActs;
        }
        ...
        if (args.flags && typeof args.flags === 'object' && !Array.isArray(args.flags)) {
            for (const [k, v] of Object.entries(args.flags)) {
                if (v && typeof v === 'object' && !Array.isArray(v) && flagPatch[k] && typeof flagPatch[k] === 'object') {
                    flagPatch[k] = { ...flagPatch[k], ...v };
                }
                else {
                    flagPatch[k] = v;
                }
            }
            notes.push('合并 flags：' + Object.keys(args.flags).join(', '));
        }
        if (Array.isArray(args.unsetFlags) && args.unsetFlags.length) {
            for (const path of args.unsetFlags) {
            ...
            notes.push('删除 flags：' + args.unsetFlags.join(', '));
        }
        ...
        if (Object.keys(sysPatch).length)
            data.system = sysPatch;
        if (Object.keys(flagPatch).length)
            data.flags = flagPatch;
        await h.callRelay('PUT', '/update', {
        ...
        try {
            await new Promise((r) => setTimeout(r, 1500));
            const raw = (await h.callRelay('GET', '/get', { query: { ...h.targetingQuery(args), uuid: fullUuid } }));
            const got = unwrapEntity(raw);
            const acts = (got?.system?.activities ?? {});
            const gFlags = (got?.flags ?? {});
            const midi = (gFlags['midi-qol'] ?? {});
            const itemacro = (gFlags.itemacro ?? {});
            const dae = (gFlags.dae ?? {});
            if (Array.isArray(args.removeActivities)) {
                for (const id of args.removeActivities) {
                    if (typeof id === 'string' && acts[id])
                        problems.push(`活动 ${id} 仍在（未被删除）`);
                }
            }
            if (pass || rawOnUse) {
                ...
                const ia = (itemacro.macro ?? {});
                const iaCmd = typeof ia.command === 'string' ? ia.command : '';
                if (!iaCmd)
                    problems.push('flags.itemacro.macro.command 未落库 → ItemMacro 取不到宏体');
                const dm = (dae.macro ?? {});
                const dmCmd = typeof dm.command === 'string' ? dm.command : '';
                if (!dmCmd)
                    problems.push('flags.dae.macro.command 未落库');
                ...
                if (wantCmd.trim()) {
                    const tailOf = (s) => s.trim().slice(-40);
            ...
            for (const path of (Array.isArray(args.unsetFlags) ? args.unsetFlags : [])) {
                let cur = gFlags;
                for (const seg of path.split('.'))
                    cur = cur?.[seg];
                if (cur !== undefined)
                    problems.push(`flags.${path} 仍存在（未删除）`);
            }
            verify = {
                activityIds: Object.keys(acts),
                activityTypes: Object.fromEntries(Object.entries(acts).map(([k, v]) => [k, v?.type])),
                onUseMacroName: midi.onUseMacroName,
                itemacroHasCommand: !!itemacro.macro?.command,
                daeHasCommand: !!dae.macro?.command,
