...
DSH 原生 FVTT 控制工具包：把 Foundry VTT 的操控能力做成 **86 个 DSH 插件工具**，
...
relay openapi 共 106 端点，包了 86 个；剩下 20 个有意不包：auth/key-request 4 个（密钥管理不交给 AI）、SSE 订阅流 6 个（指令驱动，不做监听）、headless session 4 个（与 GM 浏览器架构冲突）、截图/下载 3 个（二进制，JSON 通道装不下）、deprecated 1 个。
