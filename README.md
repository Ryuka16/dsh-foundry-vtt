...
核心思路：**模块的 WebSocket 由 GM 浏览器（云端 FVTT 页面）发起**，回连到你桌机的 relay——云端服务器零公网暴露、零安全组改动、零内网穿透。relay 把世界数据包成 HTTP API，DSH 插件再把 API 包成 88 个 AI 工具。

## 仓库内容

| 目录 | 是什么 | 说明 |
|---|---|---|
| `plugin/` | **DSH 插件（本仓库自研）** | 88 个工具（86 数据 API + 内置结构参考库 + 本地资料库检索），零静态依赖（只用 node 内置 fetch），是 AI 的「手」 |
